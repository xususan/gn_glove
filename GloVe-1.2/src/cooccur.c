...
int write_hash_dump(HASHREC **ht){
    FILE *fid = fopen(hash_dump_file, "wb");
    long long i;
    int count = 0;
...
int get_cooccurrence() {
    ...
    // Get the vocab_file from command line, format is "%1000s %lld",
    // which is a string with max len as 1000 and long long number
    ...
    // Use `fscanf` to load files line by line
...
int main(int argc, char **argv) {
    ...
    hash_dump_file = malloc(sizeof(char) * MAX_STRING_LENGTH);
    ...
    if (argc == 1) {
        ...
        printf("\t-hash-dump-file <file>\n");
        printf("\t\tFilename, ??? include extension, for the output Hash Table Dump\n");
    ...
    /* Add hash table dump file*/
    if ((i = find_arg((char *)"-hash-dump-file", argc, argv)) > 0 ) strcpy(hash_dump_file, argv[i+1]);
    else strcpy(hash_dump_file, (char *)"hashdump.txt");
